...
void kernel_main() {
    ...
    constexpr auto g0 = TensorAccessorArgs<2>();                                   const auto Aah = TensorAccessor(g0, a0, TB);
    constexpr auto g1 = TensorAccessorArgs<g0.next_compile_time_args_offset()>();  const auto Aam = TensorAccessor(g1, a1, TB);
    constexpr auto g2 = TensorAccessorArgs<g1.next_compile_time_args_offset()>();  const auto Aal = TensorAccessor(g2, a2, TB);
    constexpr auto g3 = TensorAccessorArgs<g2.next_compile_time_args_offset()>();  const auto Abh = TensorAccessor(g3, a3, TB);
    constexpr auto g4 = TensorAccessorArgs<g3.next_compile_time_args_offset()>();  const auto Abm = TensorAccessor(g4, a4, TB);
    constexpr auto g5 = TensorAccessorArgs<g4.next_compile_time_args_offset()>();  const auto Abl = TensorAccessor(g5, a5, TB);
