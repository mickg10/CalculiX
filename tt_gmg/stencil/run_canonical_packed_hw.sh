...
RUN_TAG=${2:-canonical_packed_spmv_run1}
...
# Share the marker with every brick candidate.  A failed mesh open consumes
# the boot just as surely as a completed program.
...
python3 - "${PACKED_ROOT}" "${VECTOR_TAG}" <<'PY'
import hashlib
import json
import pathlib
import sys
...
tag = sys.argv[2]
layout = json.loads((root / "canonical_packed_layout.json").read_text())
vector = json.loads((root / f"canonical_packed_vector_{tag}.json").read_text())
records = list(layout["files"].values()) + list(vector["files"].values())
checked = 0
for record in records:
    ...
    checked += 1
print(f"ARTIFACT_PREFLIGHT pass=1 files={checked} vector={tag}")
PY
artifact_rc=$?
if ((artifact_rc != 0)); then
    echo "SAFETY canonical packed artifact preflight failed"
    exit 103
fi
...
if pgrep -a -f "[m]etal_example_.*(brick_spmv|canonical_packed_spmv)"; then
...
timeout "${WORKLOAD_TIMEOUT_SECONDS}s" \
    ./build_Release/programming_examples/metal_example_canonical_packed_spmv \
    "${PACKED_ROOT}" 3 "${VECTOR_TAG}"
rc=$?
