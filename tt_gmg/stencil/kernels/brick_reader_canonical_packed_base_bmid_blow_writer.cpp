// RISC-1 reader/writer for canonical base-stencil packed groups.
//
// Packed shifted-B mid/low pages follow the same group permutation as the
// output.  The low stream keeps the qualifying run52 63/81 cadence.
...
void kernel_main() {
    ...
    const auto b1 = TensorAccessor(g0, b1_addr, tile_bytes);
    constexpr auto g1 = TensorAccessorArgs<g0.next_compile_time_args_offset()>();
    const auto b2 = TensorAccessor(g1, b2_addr, tile_bytes);
