// Selective six-product bf16x3 accumulation. All face/edge/center offsets
// retain x-low. One inversion-symmetric corner pair also retains x-low;
// the other seven corner pairs use the five-product T5 order proven by the
// host oracle. This is a default-off traffic/pack diagnostic, not run41.
...
inline bool retains_blow(uint32_t term) {
    ...
    const uint32_t manhattan =
        ...
        (dj > 1 ? dj - 1 : 1 - dj) +
        (dk > 1 ? dk - 1 : 1 - dk);
    // Offset ids 6 and 20 are (-1,+1,-1) and (+1,-1,+1).
                ...
                if (stream_term < accumulate_terms) {
                    ...
                    for (uint32_t output_component = 0;
                        ...
                        if (retain_low) {
                            ...
                            // Match brick_format.py's five-product T5 order.
                            mac_term<ckernel::MathFidelity::HiFi4>(
                                cb_a, cb_bhi, a_low, batch_lane,
                                output_component, false);
                            mac_term<ckernel::MathFidelity::HiFi4>(
