// RISC-1 half of the shifted-resident-vector brick path.
//
// Stream A-low plus direct x-mid/x-low tile pages, then drain fp32 output.
// The x-low cadence is exactly the qualifying run52 63/81 policy.
...
void kernel_main() {
    ...
    for (uint32_t group_slot = 0; group_slot < n_groups; ++group_slot) {
        ...
        for (uint32_t stream_batch = 0;
            ...
            const uint32_t pm = get_write_ptr(cb_bmid);
            const uint32_t pl = low_count == 0 ? 0 : get_write_ptr(cb_blow);
            ...
            for (uint32_t batch_lane = 0;
                ...
                noc_async_read_page(
                    b_page,
                    b1,
                    pm + batch_lane * tile_bytes);
                if (retains_blow(term)) {
                    noc_async_read_page(
                        b_page,
                        b2,
                        pl + low_index * tile_bytes);
                    ++low_index;
                }
            }
