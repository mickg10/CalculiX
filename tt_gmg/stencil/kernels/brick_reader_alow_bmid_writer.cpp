// Split-A reader/writer, RISC-1 half.
//
// Load the resident halo, stream A-low through NOC-1 while materializing the
// middle resident-x split, then drain output.  A-low has its own CB so both
// data-movement processors can publish independently and safely.
...
void kernel_main() {
    ...
    const auto halo = TensorAccessor(g1, halo_addr);
    constexpr auto g2 = TensorAccessorArgs<g1.next_compile_time_args_offset()>();
    const auto output = TensorAccessor(g2, output_addr, 4096);
    ...
    for (uint32_t group_slot = 0; group_slot < n_groups; ++group_slot) {
        ...
        for (uint32_t stream_batch = 0;
            ...
            const uint32_t pa = get_write_ptr(cb_a_low);
            const uint32_t pb = get_write_ptr(cb_bmid);
