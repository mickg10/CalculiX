// Five-product bf16x2-b MAC.  High and middle x splits arrive through
// separate CBs produced concurrently by the two data-movement RISCs.
...
void kernel_main() {
    ...
    for (uint32_t group = 0; group < n_groups; ++group) {
        ...
        for (uint32_t term = 0; term < terms; ++term) {
            ...
            cb_wait_front(cb_bhi, 1);
            cb_wait_front(cb_bmid, 1);
            if (term < accumulate_terms) {
                ...
                for (uint32_t output_component = 0;
                    ...
                    mac_term(cb_a, cb_bhi, abase + 0, 0, output_component, first);
                    mac_term(cb_a, cb_bmid, abase + 0, 0, output_component, false);
                    mac_term(cb_a, cb_bhi, abase + 1, 0, output_component, false);
                    mac_term(cb_a, cb_bhi, abase + 2, 0, output_component, false);
                    mac_term(cb_a, cb_bmid, abase + 1, 0, output_component, false);
                }
            }
            cb_pop_front(cb_a, 9);
            cb_pop_front(cb_bhi, 1);
            cb_pop_front(cb_bmid, 1);
        }
        tile_regs_commit();
