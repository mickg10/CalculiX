// Default-off changing-x RISC-1 reader/writer for exact fallback groups.
...
void kernel_main() {
    ...
    constexpr uint32_t cb_stage_mid = get_compile_time_arg_val(4);
    constexpr uint32_t cb_stage_low = get_compile_time_arg_val(5);
    constexpr uint32_t cb_page_table = get_compile_time_arg_val(6);
    constexpr uint32_t cb_lane_map = get_compile_time_arg_val(7);
    constexpr uint32_t a_tiles_per_term = 3;

    constexpr auto g0 = TensorAccessorArgs<8>();
    const auto a2 = TensorAccessor(g0, a2_addr, canonical_dynamic_page::kTileBytes);
    constexpr auto g1 = TensorAccessorArgs<g0.next_compile_time_args_offset()>();
    const auto vector1 = TensorAccessor(
        g1, vector1_addr, canonical_dynamic_page::kTileBytes);
    constexpr auto g2 = TensorAccessorArgs<g1.next_compile_time_args_offset()>();
    const auto vector2 = TensorAccessor(
        g2, vector2_addr, canonical_dynamic_page::kTileBytes);
    ...
    const auto output = TensorAccessor(g5, output_addr, 4096);

    // Dedicated fixed-address L1 scratch.  Reserve each complete allocation
    // once so get_write_ptr is valid, then never push either CB; variable
    // page_count is not a legal FIFO advance for a 62-page circular buffer.
                ...
                for (uint32_t batch_lane = 0;
                    ...
                    for (uint32_t output_component = 0;
                        ...
                        noc_async_read_page(
                            ...
                            a2,
                            a_l1 + (batch_lane * a_tiles_per_term + output_component) *
                                canonical_dynamic_page::kTileBytes);
                    }
