// Full bf16x3 accumulation with halo materialization split three ways:
// RISC-0 produces x_hi, RISC-1 produces x_mid, and the otherwise mostly-idle
// PACK TRISC produces x_lo directly into c_4.  UNPACK remains the sole CB
// consumer and PACK remains the sole producer for c_4.
...
void kernel_main() {
    ...
    for (uint32_t group = 0; group < n_groups; ++group) {
        ...
        for (uint32_t stream_batch = 0;
            ...
            // Keep all six bf16x3 products for one term adjacent at HiFi4.
            // This is the best-known correct resident-x schedule (run41).
            // Cross-term fidelity batching is deliberately absent: runs
            // 45/46/48/49 reproduced one deterministic corrupted lane despite
            // MOP reconfiguration, full LLK reinitialization, full-pipeline
            // drains, and an explicit FP32 destination clear.
            for (uint32_t batch_lane = 0;
                 batch_lane < terms_per_publish;
                 ++batch_lane) {
                const uint32_t stream_term =
                ...
                if (stream_term < accumulate_terms) {
                    ...
                    for (uint32_t output_component = 0;
                         ...
                         ++output_component) {
                        const uint32_t a_hi =
                            batch_lane * 9 + 0 * 3 + output_component;
                        const uint32_t a_mid =
                            batch_lane * 9 + 1 * 3 + output_component;
                        const uint32_t a_low =
                            batch_lane * 9 + 2 * 3 + output_component;
