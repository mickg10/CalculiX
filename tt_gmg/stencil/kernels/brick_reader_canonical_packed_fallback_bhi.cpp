// RISC-0 reader for exact dense fallback A and packed shifted-B pages.
// Fallback A has its own compact group index; B/output retain the packed
// group index after the 133 canonical base groups on each chip.
...
void kernel_main() {
    ...
    for (uint32_t group_slot = 0; group_slot < n_groups; ++group_slot) {
        ...
        for (uint32_t stream_batch = 0;
            ...
            const uint32_t b_l1 = get_write_ptr(cb_bhi);
            for (uint32_t batch_lane = 0;
                ...
                for (uint32_t output_component = 0;
                    ...
                    noc_async_read_page(
                        a_page0 + output_component,
                        a1,
                        a_l1 + (a_index + 3 + output_component) * tile_bytes);
                }
                noc_async_read_page(
                    b_page,
                    b0,
                    b_l1 + batch_lane * tile_bytes);
            }
