// Run52 arithmetic with a lossless three-type corner-coefficient stream.
// Corner A tiles are bitwise-identical to the original operator; they are
// merely shared across the three input terms of an offset.  Product order and
// the selective b-low policy are unchanged.
...
// Type 1 is every diagonal entry.  For an off-diagonal entry, the exact
// elasticity corner coefficient is type 2 when the two offset signs match
// and type 0 when they differ.  build_corner_constant_compression.py proves
// this map against every corner tile before emitting the runtime artifact.
inline uint32_t corner_coefficient_type(
    uint32_t offset_id,
    ...
    return signs[input_component] == signs[output_component] ? 2u : 0u;
}
...
void kernel_main() {
    ...
    constexpr auto cb_corner = tt::CBIndex::c_5;
    ...
    for (uint32_t group = 0; group < n_groups; ++group) {
        ...
        for (uint32_t stream_batch = 0;
            ...
            const bool corner_batch = is_corner(first_term / 3);
                ...
                PACK({
                    ...
                    for (uint32_t batch_lane = 0;
                        ...
                        const uint32_t stream_term =
                            stream_batch * terms_per_publish + batch_lane;
                        const uint32_t term =
                            ordered_term(stream_term, order_mode);
            ...
            if (corner_batch) {
                cb_wait_front(cb_corner, corner_tiles);
            } else {
            ...
            for (uint32_t batch_lane = 0;
                ...
                const uint32_t offset_id = term / 3;
                const uint32_t input_component = term - offset_id * 3;
                ...
                if (stream_term < accumulate_terms) {
                    ...
                    for (uint32_t output_component = 0;
                         output_component < 3;
                         ++output_component) {
                        const uint32_t type = corner_batch
                            ? corner_coefficient_type(
                                  offset_id,
                                  input_component,
                                  output_component)
                            : 0u;
                        const uint32_t a_hi = corner_batch
                            ? type
                            : batch_lane * 9 + output_component;
                        const uint32_t a_mid = corner_batch
                            ? 3 + type
                            : batch_lane * 9 + 3 + output_component;
                        const uint32_t a_low = corner_batch
                            ? 6 + type
                            : batch_lane * 9 + 6 + output_component;
                        const uint32_t a_cb = corner_batch ? cb_corner : cb_a;
                        mac_term<ckernel::MathFidelity::HiFi4>(
                            a_cb, cb_bhi, a_hi, batch_lane,
                            output_component, first);
                        mac_term<ckernel::MathFidelity::HiFi4>(
                            a_cb, cb_bmid, a_hi, batch_lane,
                            output_component, false);
                        mac_term<ckernel::MathFidelity::HiFi4>(
                            a_cb, cb_bhi, a_mid, batch_lane,
                            output_component, false);
                        if (retain_low) {
                            mac_term<ckernel::MathFidelity::HiFi4>(
                                a_cb, cb_blow, a_hi, low_index,
                                output_component, false);
                            mac_term<ckernel::MathFidelity::HiFi4>(
                                a_cb, cb_bmid, a_mid, batch_lane,
                                output_component, false);
                            mac_term<ckernel::MathFidelity::HiFi4>(
                                a_cb, cb_bhi, a_low, batch_lane,
                                output_component, false);
                        } else {
                            mac_term<ckernel::MathFidelity::HiFi4>(
                                a_cb, cb_bhi, a_low, batch_lane,
            ...
            if (corner_batch) {
                cb_pop_front(cb_corner, corner_tiles);
            } else {
