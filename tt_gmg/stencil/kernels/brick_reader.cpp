// Stream three bf16 A planes and form b from a core-local aligned halo.
...
void kernel_main() {
    ...
    constexpr uint32_t halo_tiles_per_group = 72;  // 9 planes x 8192 bf16 / 1024
    constexpr uint32_t halo_tiles_per_core = 288;  // four padded group slots
    ...
    for (uint32_t group_slot = 0; group_slot < n_groups; ++group_slot) {
        ...
        for (uint32_t term = 0; term < terms; ++term) {
            ...
            const uint32_t di = offset_id / 9;          // encoded 0,1,2 for -1,0,+1
