// Diagnostic-only RISC-0 reader for isolating coefficient-stream cost.
//
// It seeds the two physical nine-tile A CB windows once, then republishes the
// stale contents after each consumer pop.  B-high materialization, the full
// 81-term compute loop, halo traffic, output writes, and CB synchronization
// remain unchanged.  The result is intentionally numerically wrong and may
// run only behind BRICK_ALLOW_INCORRECT_PROFILE.  It is never gate eligible.
...
void kernel_main() {
    ...
    for (uint32_t group_slot = 0; group_slot < n_groups; ++group_slot) {
        ...
        for (uint32_t stream_batch = 0;
            ...
            const bool seed_a_window = group_slot == 0 && stream_batch < 2;
