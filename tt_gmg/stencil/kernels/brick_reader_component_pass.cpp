// Replay the resident halo once per output component so the compute kernel can
// use one fp32 accumulator plus three legal fp32 scratch tiles.
    ...
    for (uint32_t group_slot = 0; group_slot < n_groups; ++group_slot) {
        ...
        for (uint32_t output_component = 0;
            ...
            for (uint32_t term = 0; term < terms; ++term) {
                ...
                noc_async_read_page(a_page, a0, pa + 0 * tile_bytes);
                noc_async_read_page(a_page, a1, pa + 1 * tile_bytes);
                noc_async_read_page(a_page, a2, pa + 2 * tile_bytes);

                for (uint32_t split = 0; split < 3; ++split) {
                    tt_l1_ptr uint64_t* b_words =
                        (tt_l1_ptr uint64_t*)(pb + split * tile_bytes);
                    const uint32_t plane = input_component * 3 + split;
                    ...
                    for (uint32_t brick = 0; brick < 16; ++brick) {
                        ...
                        for (uint32_t i = 0; i < 4; ++i) {
                            ...
                            for (uint32_t j = 0; j < 4; ++j) {
                                ...
                                const uint32_t destination_word =
                                    brick * 16 + i * 4 + j;
